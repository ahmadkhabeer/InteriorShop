...
def notify_vendor(order):
    from_email = setting.DEFAULT_EMAIL_FROM
...
def notify_customer(order):
    ...
    msg = EmailMultiAlternatives(subject, text_content, from_email, [to_email])
    msg.attacth_alternative(html_content, 'text/html')
    msg.send()

def checkout(request, first_name, last_name, email, address, zipcode, phone, place, amount):
    order = Order.objects.create(first_name=first_name, last_name=last_name, email=email, address=address, zipcode=zipcode, phone=phone, place=place, paid_amount=amount)
